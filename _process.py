...
def _tip_amounts(ukey):
    ...
    chefCut_ind = col42.number_input('Each Chefs tip total ($)', value=chefCut/chefCount, format='%f', key=ukey+'4', disabled=True)
    col40, col41 = st.columns([1,1])
    tippingPool_Garden = col40.number_input('Garden Pool ($)', value=0.00, format='%f', key=ukey+'5')
    tippingPool_Reg = col41.number_input('Regular Pool ($)', value=(totalPool-chefCut-tippingPool_Garden), format='%f', key=ukey+'6', disabled=True)
