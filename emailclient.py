...
password = st.secrets['Streamlit_TNT']
...
def passwordreset(email_receiver, new_password):
    ...
    try:
        ...
        msg['Subject'] = 'TNT New Temporary Password'
...
def forgotusername(email_receiver, username):
    ...
    try:
        ...
        msg['From'] = email_sender
        msg['To'] = email_receiver
        msg['Subject'] = 'TNT Forgotten Username'

        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()
        server.login(email_sender, st.secrets['Streamlit_TNT'])
        server.sendmail(email_sender, email_receiver, msg.as_string())
        server.quit
