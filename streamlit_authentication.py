...
def login(auth):
    user, status, username = auth.login(max_login_attempts=5)
...
def register(auth, conf):
    try:
        ...
        email, username, user = auth.register_user(pre_authorization=True, fields=fieldval)
        if email:
            st.sidebar.success('User created successfully')
            firestore.saveconfig(conf)
            firestore.associateEmail(email, username)
...
def forgotpassword(auth, conf):
    try:
        username, email, new_random_password = auth.forgot_password()
        if username:
            firestore.saveconfig(conf)
...
def forgotusername(auth):
    try:
        username, email = auth.forgot_username()
        if username:
